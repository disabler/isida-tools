...
class MainWindow(QtGui.QMainWindow):
	...
	def regenerate(self):
		path_to_locale = locale_generate.path_to_locale
		lf = os.listdir(path_to_locale)
		self.loc_dict = {}
		self.label_inf = ''
		for tmp in lf:
			if tmp[-4:] == '.txt':
				msg, result, lz = locale_generate.regenerate(path_to_locale,tmp)
		...
		for k in self.loc_dict.keys():
			self.nontrans += [i[0] for i in enumerate(self.loc_dict[k]) if locale_generate.locale_mark in i[1] and i[1][0] != '#']
	...
	def save_locales(self, locale_for_saving=None, main_interface=True):
		...
		for k in ls:
			...
			for t in result.split('\n\n',1)[1].split('\n'):
				if locale_generate.locale_mark not in t: tmp_body.append(t)
			tmp2_body,cnt = [],0
			for t in range(0,len(tmp_body)-2):
				if tmp_body[cnt].startswith(locale_generate.file_mark) and tmp_body[cnt+2].startswith(locale_generate.file_mark): cnt += 2
